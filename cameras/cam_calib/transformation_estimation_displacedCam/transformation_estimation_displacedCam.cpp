#include "transformation_estimation_displacedCam.h"
...
int main ()
{
    ...
    sprintf(pcd1_path, "/home/huitan/codes/cam_calib/transformation_estimation_displacedCam/scene_1_1.pcd");
    sprintf(pcd2_path, "/home/huitan/codes/cam_calib/transformation_estimation_displacedCam/scene_2_1.pcd");
    ...
    pcl::PointCloud<pcl::PointXYZRGBA>::Ptr pcd2(new pcl::PointCloud<pcl::PointXYZRGBA>);
    pcl::io::loadPCDFile<pcl::PointXYZRGBA>(pcd2_path, *pcd2);

    //results from ASIFT
	sprintf(keypoint_pairs_path, "/home/huitan/codes/cam_calib/transformation_estimation_displacedCam/matchings.txt");
    ...
    for(int i = 0; i < num_keypoint_pairs; ++i){
	  double dcol1, drow1, dcol2, drow2;
	  keypoint_pairs_cin>>dcol1>>drow1>>dcol2>>drow2;
	  ...
	  int pcd1_index, pcd2_index;
	  pcd1_index = row1 * pcd1->width + col1;
	  pcd2_index = row2 * pcd2->width + col2;
	 
	  
	  if(!isnan(pcd1->points[pcd1_index].z)  && !isnan(pcd2->points[pcd2_index].z)){
		match_tgt->points.push_back(pcd1->points[pcd1_index]);   //pcd1 is old cam pose
		match_src->points.push_back(pcd2->points[pcd2_index]);   //pcd2 is new cam pose
	  }
	}
	...
	//read the transformation matrix file from old cam pose to robot world frame 
	//对于摄像头移动的情况，针对的是获得新的camera_to_robotWorld_tf然后再乘上camera和optical link的固定的坐标变换是新的camera_to_robotWorld_tf
    sprintf(pcd1_transform_path, "/home/huitan/codes/cam_calib/transformation_estimation_fixedCam_chessboard/camera_to_robotWorld_tf.txt");
    std::ifstream pcd1_transform_cin;
    pcd1_transform_cin.open(pcd1_transform_path);
	if(!pcd1_transform_cin.is_open()){
	  std::cout<<"ATTENTION: Failed to Open Transformation File!\n"<<std::endl;
	}
	...
	printf("**********transformation matrix from old cam pose to robot world frame:\n");
    std::cout<<pcd1_transform<<std::endl;
	
	//calculate transformation matrix from new camera pose to robot world frame
	Eigen::Matrix4f pcd2_transform;
	pcd2_transform = pcd1_transform * transform_srcTotgt;
	printf("**********transformation matrix from new camera pose to robot world frame:\n");
	std::cout<<pcd2_transform<<std::endl;
	
	//save the transformation matrix from new camera pose to robot world frame
    sprintf(pcd2_transform_path, "/home/huitan/codes/cam_calib/transformation_estimation_displacedCam/updated_camera_to_robotWorld_tf.txt");
    ...
    std::cout<< "updated tf camera optical to robot world frame:\n" << camToRobot << endl;
    //save as
    sprintf(optToRobot_transform_path, "/home/huitan/codes/cam_calib/transformation_estimation_displacedCam/updated_cameraOptical_to_robotWorld_tf.txt");
    ...
    int pcd_index = 182*pcd2->width+295;   //the fist number is pixel row coordinate (read from verical axis in GIMP image editor)2nd is pixel column coordinate
    Eigen::Vector4f u(pcd2->points[pcd_index].x,
                      pcd2->points[pcd_index].y,
                      pcd2->points[pcd_index].z,
                      1.0);  //u is object position vector in camera frame (in point cloud)
    Eigen::Vector4f v = pcd2_transform*u;
    std::cout << "[TEST] v is object position vector in chessboard frame: " << v << std::endl;
}
